...
* 2. Build and download the application to the WICED Sense board.
* 3. To securely update the application:
* 	3a. Sign the build output
*       wiced_sense-BCM920737TAG_Q32-rom-ram-Wiced-release.ota.bin
*    	file using WsRsaSign utility
* 	3b. Pair with a client
* 	3c. Use WsSecOtaUpgrade application to try over the air upgrade
...
void wiced_sense_initialize_l3gd20(void)
{
	...
	if(L3GD20_SetODR(L3GD20_ODR_95Hz_BW_12_5) == MEMS_SUCCESS)
...
void wiced_sense_initialize_lsm303d(void)
{
	...
	if(LSM303D_SetMagODR(LSM303D_ODR_25Hz_M) == MEMS_SUCCESS)
		ble_trace0("LSM303D_SetMagODR Successful");
	else
		ble_trace0("LSM303D_SetMagODR Failed");

	if(LSM303D_SetMagFullScale(LSM303D_FULLSCALE_8_M) == MEMS_SUCCESS)
