...
int ws_upgrade_ota_handle_data (UINT8 *data, int len)
{
    ...
    else if (ws_upgrade_state == WS_UPGRADE_STATE_DATA_TRANSFER)
    {
        ...
        if (ws_upgrade_total_offset + ws_upgrade_current_block_offset + len >= ws_upgrade_patch_len)
        {
            UINT32 length_to_copy = ws_upgrade_patch_len - (ws_upgrade_total_offset + ws_upgrade_current_block_offset);

            BT_MEMCPY (&ws_upgrade_read_buffer[ws_upgrade_current_block_offset], data, length_to_copy );

            ws_upgrade_current_block_offset += length_to_copy;
            len  -= length_to_copy;
            data += length_to_copy;
